    ...
    if(path.startsWith('/api/')){
      ...
      if(req.method==='POST' && path==='/api/tasks'){
        ...
        const t=await store.transact(s=>{
          ...
          const t={id:identifier(),revision:1,title:goal.slice(0,44),goal,preferences:text(b.preferences||'',6000,'个人要求'),criteria:text(b.criteria||'',6000,'验收条件'),status:'ready',createdAt:stamp(),updatedAt:stamp(),materials:[],plan:[],questions:[],feedback:[],artifacts:[],events:[],usage:[],sources:[],previewData:{}};
      ...
      const result=await store.transact(s=>{
        ...
        else if(req.method==='POST' && action==='feedback'){const message=text(b.message,12000,'补充说明').trim();if(!message)throw new Error('请填写补充说明。');if(t.feedback.length>=100)throw new Error('已达到 100 条补充上限。');t.feedback.push({at:stamp(),text:message});t.questions=[];t.status='ready';log(t,message,'user');}
...
server.listen(port,'127.0.0.1',()=>console.log(`共作 0.2 · http://127.0.0.1:${port}`));
