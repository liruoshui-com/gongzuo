...
[English](README.en.md) · [快速开始](#快速开始) · [使用说明](docs/guide.md) · [验证方案](docs/benchmark.md) · [路线图](docs/roadmap.md)
...
> **v0.2.0 · 早期预览。**已有多家模型接入、任务执行和成果交付代码，并通过模拟协议与浏览器测试。还没有真实模型对照结果，**不承诺节省比例或自动达到质量标准**。上图是实际界面，不是 AI 完成任务的效果证明。
...
目前没有填入未经测量的数据。欢迎一起完善 [可复现的验证方案](docs/benchmark.md) 和 [示例结果格式](benchmarks/results.example.json)。
