...
import {requestModel, publicConfig} from './provider.mjs';
...
const instructions = `你是共作的任务协作者，帮助普通用户完成真实工作、学习与产品任务。按用户目标制定简短计划，持续遵守个人要求和验收条件。
...
export function createEngine(store, getConfig) {
  ...
  async function executeTool(id, call, signal) {
    ...
    return change(id,t => {
      ...
      if(call.name === 'analyze_csv') {const m=t.materials.find(m=>m.id===args.materialId);if(!m) throw new Error('材料不存在');const result=analyzeCSV(m.content,args);const artifacts=result.artifacts.map(a=>deliver(t,a,'local-tool'));return {facts:result.facts,artifacts:artifacts.map(a=>({id:a.id,name:a.name,version:a.version}))};}
  ...
  async function run(id, c, controller) {
    ...
    const summary={goal:t.goal,preferences:t.preferences,criteria:t.criteria,feedback:t.feedback.slice(-12),plan:t.plan,materials:t.materials.map(m=>({id:m.id,name:m.name,characters:m.content.length})),artifacts:t.artifacts.slice(-20).map(a=>({id:a.id,name:a.name,version:a.version,characters:a.content.length})),capabilities:{webSearch:c.webSearch,formats:['txt','md','csv','json','html','svg']}};
    ...
    try {
      for(let turn=0;turn<c.maxCalls;turn++) {
        ...
        let result;
        try {result=await requestModel(c,{input,tools:toolDefinitions,instructions,signal:controller.signal});}
        ...
        for(const call of result.calls) {
          ...
          if(waiting) break;
          let output;
          try {output=await executeTool(id,call,controller.signal);}
