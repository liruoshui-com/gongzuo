...
const instructions = `你是共作的任务协作者，帮助普通用户完成真实工作、学习与产品任务。按用户目标制定简短计划，持续遵守个人要求、固定要求（pinnedRequirements）和验收条件。
...
export function createEngine(store, getConfig) {
  ...
  async function run(id, c, controller) {
    ...
    const summary=buildTaskContext(t,c.webSearch),runId=identifier(),reads=[];
    ...
    try {
      for(let turn=0;turn<c.maxCalls;turn++) {
        ...
        let result;
        try {result=await requestModel(c,{input,tools:toolDefinitions,instructions,signal:controller.signal,onRequest:body=>change(id,current=>appendContextRecord(current,inspectRequest(t,body,{runId,requestId:usageId,callNumber:turn+1,reads,previous:current.contextRecords?.at(-1)})))});}
  ...
  return {
    health(t) {
      const c=getConfig(),input=[{role:'user',content:JSON.stringify(buildTaskContext(t,c.webSearch))}];
      const preflight=inspectRequest(t,buildRequestBody(c,{input,tools:toolDefinitions,instructions}),{previous:t.contextRecords?.at(-1)});
